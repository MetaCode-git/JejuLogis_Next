...
export default function EstimateDetailClient({ params }: EstimateDetailClientProps) {
  ...
  // For static builds, handle dynamic routes gracefully
  React.useEffect(() => {
    // If this is a 404 page (static build limitation), redirect to admin page
    // This happens when accessing estimate IDs that weren't generated at build time
    if (typeof window !== 'undefined' && 
        window.location.pathname.includes('/admin/estimates/') && 
        estimateId && 
        isError && 
        error?.message?.includes('404')) {
      console.warn(`Estimate ${estimateId} not found in static build, redirecting to admin page`);
      setTimeout(() => {
        router.push('/admin');
      }, 2000);
    }
  }, [estimateId, isError, error, router]);
